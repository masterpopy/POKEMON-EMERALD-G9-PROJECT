...
u16 teach_move_evolving(struct pokemon* poke)
{
	...
	if (poke->padding_maybe >> 7 & 1)
	{
		...
		{
			if (poke_moveset[slot_in_learnset_table].move == MOVE_BLANK ||
				poke_moveset[slot_in_learnset_table].level == 0xFF)
				return 0;
...
u8 relearnable_moves(struct pokemon* poke, u16 moves_table[])
{
	...
	for (u8 i = 0; poke_moveset[i].move != MOVE_BLANK && poke_moveset[i].level != 0xFF; i++)
	{
		if (poke_moveset[i].level <= level)
		{
			...
			if (known_move != known_moves[0] && known_move != known_moves[1] && known_move != known_moves[2] &&
				known_move != known_moves[3] && number_of_moves < MAX_RELEARNABLE)
			{
				if (!moves_table)
				{
					number_of_moves++;
